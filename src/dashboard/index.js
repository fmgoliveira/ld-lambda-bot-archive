...
const { WebhookClient, MessageEmbed, Permissions, Collection, MessageActionRow, MessageButton } = require("discord.js")
const dataDir = path.join(process.cwd(), "src/dashboard").replaceAll("\\", "/")
...
module.exports = async (client) => {
    ...
    const ticketsValid = async (data, settings, guild, req, ticketPanels, ticketPanelsDb) => {
        if (Object.prototype.hasOwnProperty.call(data, "ticketsSettingsSave")) {
            ...
            if (data.ticketsClosedCategory !== "null" && data.ticketsClosedCategory) {
                let ticketsClosedCategoryValid = await guild.channels.cache.find((ch) => ch.id === data.ticketsClosedCategory)

                if (ticketsClosedCategoryValid) {
                    settings.tickets.closedCategory = data.ticketsClosedCategory
                    await settings.save()
                } else {
                    return {
                        type: "danger",
                        msg: "The tickets closed category specified was not found on this guild."
                    }
                }
            } else {
                if (data.ticketsClosedCategory === "null") {
                    settings.tickets.closedCategory = null
                    await settings.save()
                }
            }
    ...
    const ticketsPanelValid = async (data, settings, guild, req) => {
        if (Object.prototype.hasOwnProperty.call(data, "ticketPanelEdit")) {
            ...
            client.db.ticketPanels.findOneAndUpdate({ _id: req.params.panelId }, {
                guildId: guild.id,
                category: data.ticketPanelsCategory ? data.ticketPanelsCategory : category,
    ...
    const loggingValid = async (data, settings, guild, req) => {
        if (Object.prototype.hasOwnProperty.call(data, "moderationSave")) {
            ...
            if (!data.channelUpdate) data.channelUpdate = false
            ...
            if (data.channelUpdate === "on") {
                settings.logging.active.serverEvents.channelUpdate = true
                await settings.save()
            } else {
                settings.logging.active.serverEvents.channelUpdate = false
                await settings.save()
            }
            ...
            if (data.roleUpdate === "on") {
                settings.logging.active.serverEvents.roleUpdate = true
                await settings.save()
            } else {
                settings.logging.active.serverEvents.roleUpdate = false
                await settings.save()
            }
    ...
    app.get("/support", (req, res) => {
        res.redirect(process.env.SERVER_LINK)
    })
    ...
    app.get("/invite", (req, res) => {
        res.redirect("https://discord.com/api/oauth2/authorize?client_id=900398063607242762&permissions=8&scope=bot%20applications.commands")
    })
