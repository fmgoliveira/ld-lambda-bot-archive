...
module.exports = class extends Event {
    ...
    run = async (oldRole, newRole) => {
        const settings = await this.client.db.guilds.findOne({ guildId: newRole.guild.id }) || new this.client.db.guilds({ guildId: newRole.guild.id })
        ...
        try {
            newRole.guild.channels.cache.get(logging.channel.serverEvents).send({
                embeds: [
                    new MessageEmbed()
